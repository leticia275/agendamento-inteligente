...
export default async function MinhaAgendaPage() {
  ...
  const noShowRate  = completed + noShow > 0 ? ((noShow / (completed + noShow)) * 100).toFixed(0) : "—";

  return (
    <div className="min-h-screen bg-background">
      <BrandHeader />
      <main className="max-w-3xl mx-auto px-4 py-8">
        <h1 className="text-xl font-semibold text-zinc-900 mb-2">Minha agenda</h1>
        <p className="text-sm text-zinc-500 mb-8">Seus agendamentos e métricas pessoais</p>
        ...
        <div className="flex flex-col gap-3">
          {appointments.length === 0 && (
            <p className="text-sm text-zinc-400 text-center py-12">Nenhum agendamento ainda.</p>
          )}
          {appointments.map((a) => {
